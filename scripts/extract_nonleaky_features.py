...
- depMap gene-level CNV (genome-wide, not AA amplicon CN)
- depMap gene expression (RNA-seq)
- hi-C topology features (reference-based chromatin context)
- fragile site proximity (from reference genome)
Does NOT use:
...
def add_hic_features(features_df: pd.DataFrame, hic_features: dict) -> pd.DataFrame:
    ...
    # CNV × Hi-C long-range interactions
    ...
    # summary statistics using Hi-C weights
    ...
    # Hi-C summary features (as reference context)
...
def load_ecdna_labels(data_dir: Path):
    ...
    # We're NOT using any AA_* columns here
...
def extract_cnv_features(cnv: pd.DataFrame, sample_ids: list) -> pd.DataFrame:
    ...
    # fraction of genome with high CN
