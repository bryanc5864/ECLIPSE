...
Extracts:
- sequence features from reference genome (GC content, k-mer frequencies)
- fragile site proximity features
- copy number features from DepMap
- topology features from Hi-C (if available)
"""
...
class SequenceFeatureExtractor:
    ...
    def extract(self, chrom: str, start: int, end: int, output_dim: int = 256) -> np.ndarray:
        ...
        # K-mer frequencies (use k=3 for speed: 64 features)
...
class FragileSiteFeatureExtractor:
    ...
    def extract(self, chrom: str, start: int, end: int, output_dim: int = 64) -> np.ndarray:
        ...
        # is region inside a fragile site?
